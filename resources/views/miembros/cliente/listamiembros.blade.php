			...
			<table id="clientestable" width="100%">
               ...
               @foreach ($clientes as $cat)<?php $cont++;?>
				<tr>
					...
					<td><small>{{ $cat->sexo}}/<?php echo calcular_edad($cat->fnacimiento); ?></small></td>
					<td><small><?php echo date("d-m-Y",strtotime($cat->fecha_inicio)); ?></small></td>
					<td><small><small> <?php echo substr( $cat->direccion, 0, 30 ); ?></small></small></td>
					<td><small>{{ $cat->tel_emergencia}} {{ $cat->contacto}}</small></td>
